...
from . import script, voice, captions, visuals, assemble, upload, state, stock, branding, music
...
def run_once(publish_at: str | None = None, upload_to_youtube: bool = True) -> dict:
    ...
    _log("4/9 Fetching footage")
    n_clips = len(data["scenes"]) * 2
    existing = stock.take(n_clips)
    scene_videos = existing
    if len(existing) < n_clips:
        need = n_clips - len(existing)
        _log(f"    {len(existing)} existing + {need} from Pexels")
        fresh = visuals.fetch_for_scenes(data["scenes"], work / "broll")
        scene_videos = existing + fresh[:need]
        stock.scan()
    else:
        _log(f"    {len(existing)} existing stock clips")
